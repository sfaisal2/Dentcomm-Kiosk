...
function withBadges(patient) {
  ...
  const homeFormsComplete = !!(
    state.forms?.medicalHistory?.completed &&
    state.forms?.healthQuestionnaire?.completed &&
    state.forms?.hipaaAcknowledgment?.completed
  );
  const consentComplete = !!(state.consentSignatures?.financialPolicy && state.consentSignatures?.treatmentConsent);
  const formsStatus = homeFormsComplete && consentComplete ? "complete" : "partial";

  const insuranceStatus = ["verified", "previously_verified"].includes(state.dentverify?.status)
    ? "verified"
    : state.dentverify?.status === "pending"
    ? "pending"
    : "not_started";

  const needsAttention =
    isPastNoShowWindow(patient, settings) ||
    !!state.idScan?.needsStaffReview ||
    !!state.insuranceScan?.needsStaffReview ||
    state.dentverify?.status === "failed" ||
    state.dentverify?.status === "requires_reverification";
...
router.get("/pre-arrival", (req, res) => {
  const preArrivalPatients = patients
    .filter((patient) => patient.clinicId === req.clinicId)
...
router.get("/no-show-archive", (req, res) => {
  res.json(
    patients
      .filter((patient) => patient.clinicId === req.clinicId)
      .filter((patient) => patient.status === "no_show" && !patient.archived)
      .map(withBadges)
  );
});
